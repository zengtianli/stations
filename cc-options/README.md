...
```
cc-options/
├── lib_greeks.py           # 期权 Greeks 计算（FastAPI /api/scenarios 共用）
├── st_snapshot.py          # SnapTrade 拉持仓 → data/portfolio.json
├── st_activities.py        # SnapTrade 拉交易流水 → data/activities.json
├── phase2_equity_curve.py  # 重建 NLV 曲线 → data/daily_nlv.csv（含 SPY/QQQ/TQQQ）
...
- `.gitignore` 已排除 `.env` / `.env.*` / `data/` / `.DS_Store`
...
4. `sync-data.sh` — 白名单 rsync 前 3 个 → VPS
...
日常只需等 17:00 LaunchAgent 自动跑，前端读新数据自动刷新（无需重新 build，除非 Next 代码改了）。

## 数据源

- **SnapTrade**（Robinhood 资产）→ 持仓 + 交易流水
- benchmark 价格：Yahoo Finance chart API（SPY / QQQ / TQQQ）
- 凭证读 `~/Dev/stations/cc-options/.env`（不入 git / VPS）

## 依赖

- 本地：Python 3.14+，`uv` 管理 `.venv`
- 本地数据脚本：snaptrade-client / scipy（Greeks）
- VPS FastAPI：fastapi / uvicorn（见 `web-stack/services/cc-options/pyproject.toml`）
