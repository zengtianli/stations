...
- 已有 augmentproxy 反代地址 + TIAN- 卡密（自部署或卖家发的）
...
**不要跑 `auggie login`** —— 那走的是官方 OAuth 流程，配反代会被域校验拦下来。

直接写 `~/.augment/session.json`：

```bash
mkdir -p ~/.augment
cat > ~/.augment/session.json <<'EOF'
{
  "accessToken": "TIAN-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
...
字段说明：
- `accessToken`：augmentproxy 卡密本身就是 token（`TIAN-` 开头），无需另外换
