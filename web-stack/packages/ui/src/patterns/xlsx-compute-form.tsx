...
export interface XlsxComputeParam {
  name: string
  label: string
...
export interface XlsxComputeHeader {
  key: string
  label: string
}

export interface XlsxComputeResult {
  filename: string
  url: string
  size: number
  seconds: number
...
export interface XlsxComputeFormProps {
  /** Absolute base like `http://127.0.0.1:8612` or empty string for same-origin. */
  apiBase: string
  /** POST endpoint path, defaults to `/api/compute`. */
  computePath?: string
  /** GET endpoint that returns a sample input xlsx. Pass `null` to hide the button. */
  samplePath?: string | null
  /** Label for the submit button. */
  runLabel?: string
  /** Prefix for the generated result filename (will be suffixed with timestamp + `.xlsx`). */
  resultPrefix?: string
  /** File input `accept` attribute, defaults to xlsx. */
  accept?: string
  /** Extra multipart form fields (dropdowns). Rendered as tab-style buttons. */
  params?: readonly XlsxComputeParam[]
  /** Which response headers to surface in the result panel (values will be `decodeURIComponent`-ed). */
  headers?: readonly XlsxComputeHeader[]
...
/**
 * Upload-xlsx → POST multipart → download-xlsx flow extracted from the
 * hydro-reservoir pilot. Default assumptions:
 *   - Field name for file upload is `file`.
 *   - Extra params are submitted as matching form fields.
 *   - Response is an xlsx blob; CJK metadata comes back via headers
 *     as URL-encoded strings (see `X-Up-Res` / `X-Down-Res`).
 */
export function XlsxComputeForm({
  apiBase,
  ...
  async function downloadSample() {
    ...
    try {
      ...
      a.href = url
      a.download = `${resultPrefix}_sample_input.xlsx`
  ...
  async function onCompute(e: FormEvent) {
    ...
    try {
      ...
      setResult({
        filename: `${resultPrefix}_${timestampSlug()}.xlsx`,
        url,
        size: blob.size,
      ...
      <LiquidGlassCard className="p-6 md:p-8">
        ...
        {!result && (
          <div className="text-[#86868b] text-sm">
            ...
            返回 xlsx blob；header 里的 CJK 值请用{" "}
...
export default XlsxComputeForm
