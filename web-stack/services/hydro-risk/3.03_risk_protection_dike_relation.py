...
# -*- coding: utf-8 -*-
"""
保护片堤段对应关系信息导入脚本 - 从 GeoJSON 导入到风险分析 Excel
...
【使用方法】
    ...
    # 指定路径
    python3 3.03_*.py -g /path/to/data.geojson -e /path/to/risk.xlsx

【功能】
    1. 从 input/geojson/dd_fix.geojson 读取堤段数据
    2. 提取保护片编码(polder_id)和堤段编码(ds_code)的对应关系
    3. 参考模板的字段结构
    4. 填充到 output/risk_xx.xlsx 的"保护片堤段对应关系信息" sheet
    5. 自动备份原文件
    6. 生成详细报告
...
【字段说明】
    - polder_code: 保护片编码（从 polder_id 提取，规范为大写，如SX0003）
    - ds_code: 堤段编码（从 ds_code 提取，如20250001）
"""
import argparse
import pandas as pd
import json
...
def fill_polder_dike_relation(geojson_data, excel_path, sheet_name='保护片堤段对应关系信息'):
    """
    将 GeoJSON 数据填充到 Excel 的保护片堤段对应关系信息 sheet
    ...
    for idx, feature in enumerate(features):
        ...
        # 跳过无效记录
        if not polder_code or not ds_code:
            continue
...
# from common import normalize_code, read_geojson, create_backup

# from common import normalize_code, read_geojson, create_backup
def generate_report(excel_path, geojson_path, filled_count, polder_count, backup_path):
    """生成填充报告"""
    df = pd.read_excel(excel_path, sheet_name='保护片堤段对应关系信息', header=1)
...
"""
    ...
    report_path = os.path.join(report_dir, '保护片堤段对应关系填充报告.txt')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='保护片堤段对应关系填充 - 从 GeoJSON 导入到风险分析 Excel',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='示例: python3 3.03_risk_protection_dike_relation.py -g /path/to/dd.geojson -e /path/to/hx.xlsx'
    )
    parser.add_argument('-g', '--geojson', help='GeoJSON 文件路径 (dd.geojson)')
    parser.add_argument('-e', '--excel', help='Excel 文件路径 (risk_xx.xlsx)')
    parser.add_argument('-s', '--sheet', help='Sheet 名称', default='保护片堤段对应关系信息')
    ...
    try:
        ...
        # 3. 填充数据
        filled_count, polder_count = fill_polder_dike_relation(geojson_data, excel_path, sheet_name)
